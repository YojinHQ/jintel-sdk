...
PUBLISHED=$(npm view @yojinhq/jintel-mcp version 2>/dev/null || echo "0.0.0")
...
if [ -z "$BUMP" ]; then
  echo ""
  echo "Usage: pnpm publish:mcp <patch|minor|major> [--otp CODE]"
  echo ""
  echo "  patch  — bug fixes (0.3.0 -> 0.3.1)"
  echo "  minor  — new features, backwards-compatible (0.3.0 -> 0.4.0)"
...
CURRENT_ON_NPM=$(npm view "@yojinhq/jintel-mcp@$CURRENT" version 2>/dev/null || true)
...
if [ "$SKIPPED_BUMP" = "0" ]; then
  ...
  git commit -m "chore: bump jintel-mcp to v$NEW_VERSION
